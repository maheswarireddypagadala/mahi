...
public class getTextAttributeLinksGoogle {
	...
	public static void main(String[] args) {
		...
		String signihref=signin.getAttribute("href");
		System.out.println("signihref:" + signihref);
		
		By linktagl=By.tagName("a");
		List <WebElement>homepagelinks=driver.findElements(linktagl);
		int googlehomepagecount=homepagelinks.size();
		System.out.println("googlehomepagecount : "+ googlehomepagecount);
		
		 for(int index=0;index<homepagelinks.size();index++)
		 {
		    String linknames=homepagelinks.get(index).getText();
			System.out.println(index + " " +linknames);
		}
